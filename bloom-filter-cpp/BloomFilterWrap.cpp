...
void BloomFilterWrap::Init(Local<Object> exports) {
  ...
  // NODE_SET_PROTOTYPE_METHOD(tpl, "dummy", BloomFilterWrap::Dummy);
  NODE_SET_PROTOTYPE_METHOD(tpl, "getHashes", BloomFilterWrap::GetHashes);
  // NODE_SET_PROTOTYPE_METHOD(tpl, "add", BloomFilterWrap::Add2);
...
// void BloomFilterWrap::Dummy(const FunctionCallbackInfo<Value>& args) {
//   Isolate* isolate = args.GetIsolate();
//   String::Utf8Value str(isolate, args[0]->ToString(isolate->GetCurrentContext())
//       .FromMaybe(v8::Local<v8::String>()));
//   const char * buffer = *str;
...
void BloomFilterWrap::GetHashes(const FunctionCallbackInfo<Value>& args) {
  Isolate* isolate = args.GetIsolate();
  String::Utf8Value str(isolate, args[0]->ToString(isolate->GetCurrentContext())
      .FromMaybe(v8::Local<v8::String>()));
  const char* buffer = *str;
  ...
  BloomFilterWrap* obj = ObjectWrap::Unwrap<BloomFilterWrap>(args.Holder());
  std::vector<int> hashes = obj->getHashes(buffer);

  // int* hashes = obj->getHashes(buffer);
  // for(int i = 0; i < )
  // Array arr = Array::New(isolate, hashes.size());
  Local<Array> arr = Array::New(isolate);
  for (unsigned int i = 0; i < hashes.size(); i++) {
    Local<Object> result = Object::New(isolate);
    result->Set(String::NewFromUtf8(isolate, "val"), Number::New(isolate, hashes[i]));
    arr->Set(i, result);
  }


  // args.GetReturnValue().Set(Int32Array::New(isolate, hashes));
